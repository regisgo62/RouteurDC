...
int teAff = 0;
int mirror=0;  // 0 pour sens direct, 13 pour sens inverse
...
void RAAfficheurClass::setup()
{
  ...
  if (mirror!=0) display.flipScreenVertically();
  display.setFont(ArialMT_Plain_10);
  cls();
  affiche(13+mirror, "bidouilleurs");
  affiche(23+mirror, "Reseau");
  affiche(31+mirror, "   -tonome");
#ifdef WifiServer
  if (!SAP)
    affiche(39+mirror, WiFi.localIP().toString());
  else
    affiche(39+mirror, WiFi.softAPIP().toString());
#endif

#ifdef Bluetooth
  affiche(50+mirror, "Bluetooth");
#endif
  delay(2000);
}
...
//  display.drawString(35, li, a);
 display.drawString(32, li-13+mirror, a);
...
void RAAfficheurClass::affichage_oled()
{
 ...
 //   delay(100);
  ...
  if (teAff >= 5)
  {
    ...
    display.drawProgressBar(32, 50-13+mirror, 60, 10, abs(puissanceGradateur) / 10);
    // draw the percentage as String
    display.setTextAlignment(TEXT_ALIGN_LEFT);
    display.setFont(ArialMT_Plain_10);
    display.drawString(32, 13-13+mirror, "I = " + String(intensiteDC) + " A  ");
    display.drawString(32, 24-13+mirror, "U = " + String(capteurTension) + " V  ");
#ifdef MesureTemperature
    display.drawString(32, 35-13+mirror, "T = " + String(temperatureEauChaude) + " °C  ");
#endif 
/*
...
void RAAfficheurClass::affichage_oled(){
 if (teAff>=5) {
  ...
  Heltec.display->drawString(4, 0, "I_bat = " + String(intensiteDC) + " A  ");  

 ...
 float floattension=0;
  ...
  { // boucle exterieure
    ...
    { // boucle interieur
      ...
      intensiteDC = intPince - routeur.zeropince;                     // decalage du zero
      intensiteDC = (intensiteDC)*routeur.coeffPince;           // applique le coeff
      intensiteDC = (intensiteDC + (imax - 1) * pince1) / imax; // intégration de la mesure
      pince1 = intensiteDC;
    }

    intensiteDC = ((jmax - 1) * pince2 + intensiteDC) / jmax;
    pince2 = intensiteDC;
  ...
  int Tmax=20000; //us periode 50hz = 20ms = 20 000 us
    ...
    int pause=0;
       int i;
        ...
        start_times = micros();
        for(i=0;i<ECH;i+=1) { valuesA[i] = analogRead(pinPinceAC); valuesB[i] = analogRead(pinPinceACref); }  
        stop_times = micros();
      ...
      for (int u=0;u<nbfois;u++)
      {
        ...
        for(i=0;i<ECH;i+=1) {
            valuesA[i] = analogRead(pinPinceAC); valuesB[i] = analogRead(pinPinceACref);

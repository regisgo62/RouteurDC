...
int RARegulationClass::regulGrad(int dev)
{
 ...
 // courant décroissant
  if (dev < 0) 
  {
    ...
    if ((devcount > 5) && (intensiteDC > 2)) calPuis -= variation_rapide;
  }*/

   // descente réguliere lente 
    if ((devlente == 1) && (intensiteDC > 0))
    {
    if (intensiteDC > 2)
        {
  ...
  if (devlente == 0) {
            ...
            if ((intensiteDC < 2) && (intensiteDC >= 0) ) 
                { 
                  chargecomp=1;  // fin de charge
                  calPuis += variation_normale;
                }
          if ((intensiteDC < 0) && (intensiteDC >= -routeur.toleranceNegative) ) 
                { 
                  chargecomp=1;  // fin de charge
 ...
 // courant devient négatif
   if (intensiteDC < -routeur.toleranceNegative)
        {
         if (chargecomp==0) calPuis = calPuis/2; else calPuis -= variation_normale;
         if ((ineg < 1000) && (intensiteDC < 0))
            {
              ineg++;
              //puisGradmax = calPuis-2*variation_rapide;
              if (intensiteDC < -routeur.toleranceNegative) calPuis  -= variation_normale;
            } 
...
int RARegulationClass::pilotage()
{
  ...
  if ((marcheForcee) && (tempo == 1))
  {
    ...
    { // decremente toutes les minutes
      tempdepart = millis();
      marcheForceeTemporisation--;
      paramchange = 1;
    }                       // durée de forcage en milliseconde
    if (marcheForceeTemporisation == 0) // fin de temporisation
    {
      marcheForcee = false;
      tempo = 0;
    }
